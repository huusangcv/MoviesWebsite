import img1 from "../../assets/images/poster1.jpg";
import img2 from "../../assets/images/poster2.jpg";
import img3 from "../../assets/images/poster3.jpg";
import img4 from "../../assets/images/poster4.jpg";
import img5 from "../../assets/images/poster5.jpg";
import img6 from "../../assets/images/poster6.jpg";
import img7 from "../../assets/images/poster7.jpg";
import poster1 from "../../assets/images/poster1.jpg";
import poster2 from "../../assets/images/poster2.jpg";
import poster3 from "../../assets/images/poster3.jpg";
import poster4 from "../../assets/images/poster4.jpg";
import poster5 from "../../assets/images/poster5.jpg";
import poster6 from "../../assets/images/poster6.jpg";
import poster7 from "../../assets/images/poster7.jpg";
const movieData = [
    // {
    //     id: 1,
    //     slug: "deadpool-va-wolverine",
    //     poster: poster1,
    //     author: "MOVIE",
    //     title: "Deadpool & Wolverine",
    //     topic: "Deadpool và Wolverine",
    //     description: 'Đôi bạn "thân" lần đầu tiên kết hợp trên màn ảnh!',
    //     thumbnailImage: img1,
    //     thumbnailTitle: "Deadpool & Wolverine",
    //     thumbnailDescription: "Deadpool và Wolverine",
    // },
    {
        id: 2,
        slug: "cuu-long-thanh-trai-vay-thanh",
        poster: poster2,
        author: "MOVIE",
        title: "Twilight of the Warriors: Walled In",
        topic: "Cửu Long Thành Trại: Vây thành",
        description:
            "Cửu Long Thành Trại là khu vực phức tạp có rất nhiều băng đảng xã hội đen. Khi Lạc Quân nhập cảnh trái phép vào Hồng Kông, anh được Quyền Phong giúp đỡ để tồn tại ở nơi này. Trong lúc gặp nạn, họ vô tình phát hiện ra quy luật ngầm giữa sự hỗn loạn. Đứng trước những thế lực tội ác, họ đứng lên chống lại âm mưu đen tối của kẻ thù để giữ lời thề bảo vệ sự bình yên.",
        thumbnailImage: img2,
        thumbnailTitle: "Twilight of the Warriors: Walled In",
        thumbnailDescription: "Cửu Long Thành Trại: Vây thành",
    },
    {
        id: 3,
        slug: "vung-dat-cam-lang-ngay-mot",
        poster: poster3,
        author: "MOVIE",
        title: "A Quiet Place: Day One",
        topic: "Vùng đất câm lặng: Ngày một",
        description: "Chứng kiến ngày mà cả thể giới bỗng im lặng",
        thumbnailImage: img3,
        thumbnailTitle: "A Quiet Place: Day One",
        thumbnailDescription: "Vùng đất câm lặng: Ngày một",
    },
    {
        id: 4,
        slug: "gia-toc-rong-phan-2",
        poster: poster4,
        author: "MOVIE",
        title: "House of the Dragon",
        topic: "Gia Tộc Rồng",
        description:
            "Lấy bối cảnh ở vương triều Targaryen, thế hệ được trị vì bởi cha ông Daenerys Targaryen. Khi này, ông tổ Daenerys là Aegon người đã ra lệnh cho rồng xâm lăng rồi thống nhất Bảy Vương Quốc. Nhưng cuộc chiến bỗng trở nên khốc liệt hơn khi mà Vũ điệu của bầy Rồng xảy ra, lợi dụng vào tình thế đó hai chị em Targaryen đã lên kế hoạch tranh giành ngôi báu mà Aegon để lại.",
        thumbnailImage: img4,
        thumbnailTitle: "House of the Dragon",
        thumbnailDescription: "Gia Tộc Rồng",
    },
    {
        id: 5,
        slug: "hanh-tinh-khi-vuong-quoc-moi",
        poster: poster5,
        author: "MOVIE",
        title: "Kingdom of the Planet of the Apes",
        topic: "Hành tinh khỉ: Vương quốc mới",
        description:
            "Đạo diễn Wes Ball mang một hơi thở mới đến loạt phim điện ảnh danh tiếng này. Lấy bối cảnh nhiều đời sau sự ra đi của Vua Khỉ Caesar, hành tinh này là nơi loài khỉ thống trị, còn loài người dần lui về trong bóng tối. Khi một thủ lĩnh khỉ có tên Proximus Caesar xây dựng đế chế của mình, chú khỉ trẻ tuổi Noa bắt đầu một hành trình gian khổ, đặt câu hỏi về tất cả những gì anh ta đã biết về quá khứ và đưa ra những lựa chọn định hình lại tương lai cho cả loài khỉ và loài người.",
        thumbnailImage: img5,
        thumbnailTitle: "Kingdom of the Planet of the Apes",
        thumbnailDescription: "Hành tinh khỉ: Vương quốc mới",
    },
];
export default movieData;
